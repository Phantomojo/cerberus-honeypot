...
static error_template_t error_templates[] = {
    ...
    {"ping",
     {"ping: unknown host example.com",
      ...
      NULL},
     3},
    {"ifconfig", {"ifconfig: command not found", "No such device", NULL, NULL, NULL}, 2},
    {"su",
     {"su: Authentication failure", "su: incorrect password", "su: Permission denied", NULL, NULL},
     3},
    {"sudo",
...
static const session_behavior_t router_behavior = {.response_variance = 0.3f,
                                                   .min_delay_ms = 50,
                                                   .max_delay_ms = 500,
                                                   .timeout_seconds = 300,
...
static const session_behavior_t camera_behavior = {.response_variance = 0.4f,
                                                   .min_delay_ms = 100,
                                                   .max_delay_ms = 800,
                                                   .timeout_seconds = 180,
...
command_behavior_t generate_command_behavior(const char* command) {
    command_behavior_t behavior = {.execution_delay_ms = 50 + (rand() % 450),
    ...
    if (strstr(command, "ping") || strstr(command, "ssh") || strstr(command, "scp") ||
        strstr(command, "wget")) {
...
int apply_behavioral_variations(char* output, size_t output_size, command_behavior_t* behavior) {
    if (!output || output_size < 64)
        return -1;
    if (!behavior || !behavior->returns_error)
        return 0;
    ...
    snprintf(output,
             ...
             "%s: %s (exit code: %u)",
             error,
             behavior->error_message ? behavior->error_message : "Unknown error",
             behavior->error_code);
...
const char* get_realistic_error(const char* command) {
    ...
    // Generic errors
    static const char* generic_errors[] = {"command not found",
                                           "Permission denied",
...
const char* get_timeout_error(const char* command) {
    ...
    static const char* timeouts[] = {"Connection timed out",
                                     "Operation timed out",
                                     "Read timed out",
...
const char* get_permission_error(const char* command, const char* path) {
    ...
    static const char* perm_errors[] = {"Permission denied",
...
int generate_error_response(const char* command, char* output, size_t output_size) {
    if (!command || !output || output_size < 128)
        return -1;
...
float get_jitter_factor(session_behavior_t* behavior) {
    if (!behavior || !behavior->has_jitter)
        return 1.0f;
 ...
 */
int add_behavioral_delays(char* output,
                          size_t output_size,
...
int vary_error_format(const char* base_error, char* output, size_t output_size) {
    if (!base_error || !output || output_size < 128)
        return -1;
