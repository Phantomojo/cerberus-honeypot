...
static void init_users(system_state_t* state) {
    ...
    snprintf(root->password_hash,
             sizeof(root->password_hash),
             "$6$%08X$%032X",
             state_rand(state),
             state_rand(state));
    root->uid = 0;
    ...
    struct {
        const char* name;
        uid_t uid;
    } sys_users[] = {{"daemon", 1}, {"bin", 2}, {"sys", 3}, {"nobody", 65534}};
...
static void init_processes(system_state_t* state) {
    ...
    const char* kernel_threads[] = {
        "init", "kthreadd", "ksoftirqd/0", "kworker/0:0", "kswapd0", "watchdog/0", "kdevtmpfs"};
    ...
    for (int i = 0; i < services_to_add && state->process_count < MAX_STATE_PROCESSES; i++) {
        ...
        if (state->next_pid > 32000)
            state->next_pid = state_rand_between(state, 1000, 2000);

        proc->ppid = 1; /* Child of init */
        proc->uid = (strcmp(services[i].name, "dropbear") == 0)
                        ? 0
...
static void init_network(system_state_t* state) {
    ...
    for (int i = 0; i < 3 && state->connection_count < MAX_STATE_CONNECTIONS; i++) {
        if (i == 1 && state_rand_between(state, 0, 100) < 30)
            continue; /* Sometimes no telnet */
        ...
        conn->owner_pid =
            (state->process_count > 0)
                ? state->processes[state_rand_between(state, 0, state->process_count - 1)].pid
                : 1;
    }

    /* Camera: RTSP on 554 */
    if (state->profile.type == DEVICE_TYPE_CAMERA &&
        state->connection_count < MAX_STATE_CONNECTIONS) {
    ...
    /* Router: DNS on 53 */
    if (state->profile.type == DEVICE_TYPE_ROUTER &&
        state->connection_count < MAX_STATE_CONNECTIONS) {
...
static void init_filesystem(system_state_t* state) {
    ...
    root_mount->used_kb =
        state_rand_between(state, root_mount->total_kb * 60 / 100, root_mount->total_kb * 80 / 100);
    ...
    strncpy(
        proc_mount->options, "rw,nosuid,nodev,noexec,relatime", sizeof(proc_mount->options) - 1);
...
static void calculate_memory_usage(system_state_t* state) {
    ...
    uint32_t kernel_overhead =
        state->profile.total_ram_kb * state_rand_between(state, 10, 20) / 100;
...
static void calculate_load_average(system_state_t* state) {
    ...
    if (state->load_avg_1 > 500)
        state->load_avg_1 = state_rand_between(state, 10, 100);
    if (state->load_avg_5 > 400)
        state->load_avg_5 = state_rand_between(state, 10, 80);
    if (state->load_avg_15 > 300)
        state->load_avg_15 = state_rand_between(state, 5, 60);
}
...
int state_engine_init(system_state_t* state, const device_profile_t* profile) {
    ...
    switch (state->profile.type) {
        ...
        case DEVICE_TYPE_DVR:
            ...
            break;
        default:
            prefix_idx = state_rand_between(state, 3, 4);
            break;
    }
    snprintf(state->hostname,
             sizeof(state->hostname),
             "%s-%04X",
             prefixes[prefix_idx],
             state_rand_between(state, 0, 0xFFFF));
...
int state_engine_morph(system_state_t* state, uint32_t seed) {
    ...
    if (state->profile.type == DEVICE_TYPE_ROUTER)
        prefix_idx = 0;
    else if (state->profile.type == DEVICE_TYPE_CAMERA)
        prefix_idx = 1;
    snprintf(state->hostname,
             sizeof(state->hostname),
             "%s-%04X",
...
int state_generate_shadow(system_state_t* state, char* buf, size_t size) {
    ...
    for (int i = 0; i < state->user_count && written < (int)size - 150; i++) {
        ...
        int len = snprintf(
            buf + written, size - written, "%s:%s:18000:0:99999:7:::\n", u->username, hash);
        if (len > 0)
            written += len;
    }
...
int state_generate_df_output(system_state_t* state, char* buf, size_t size) {
    ...
    int written = snprintf(
        buf, size, "Filesystem           1K-blocks      Used Available Use%% Mounted on\n");
