...
network_config_t* create_secure_network_config(const char* base_ip) {
    ...
    if (sec_validate_interface_name(iface_name) != SEC_VALID) {
        log_event_level(LOG_ERROR,
                        "create_secure_network_config: Invalid interface name generated");
        free(config);
        return NULL;
    }
    ...
    log_event_level(LOG_INFO,
                    "create_secure_network_config: Network configuration created successfully");
    return config;
}
        ...
        for (int i = 0; i < config->interface_count; i++) {
            if (sec_validate_numeric_range(config->interfaces[i].mtu, 576, 9000) != SEC_VALID) {
                log_event_level(LOG_WARN,
                                "generate_secure_interface_variations: Invalid MTU detected, "
                                "resetting to 1500");
