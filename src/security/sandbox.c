...
bool is_valid_sandbox_user(const char* username) {
    ...
    const char* valid_users[] = {
        "nobody", "daemon", "cowrie", "rtsp", "www-data",
...
bool validate_sandbox_config(const sandbox_config_t* config) {
    ...
    if (config->max_memory_mb > 0 && 
        (config->max_memory_mb < SANDBOX_MIN_MEMORY_LIMIT || 
    ...
    if (config->max_cpu_percent > 0 && 
        (config->max_cpu_percent < SANDBOX_MIN_CPU_LIMIT || 
    ...
    if (strlen(config->chroot_path) > 0 && 
        !is_safe_chroot_path(config->chroot_path)) {
        return false;
    }
...
void log_sandbox_event(const char* service_name, const char* event, const char* details) {
    char log_msg[512];
    snprintf(log_msg, sizeof(log_msg), "SANDOX [%s]: %s - %s", 
             service_name, event, details ? details : "");
