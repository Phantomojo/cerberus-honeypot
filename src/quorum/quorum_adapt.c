...
__attribute__((unused)) static const char* brute_force_signatures[] = {
    "Failed password for", "Invalid user", "Received disconnect", "Connection refused"};

__attribute__((unused)) static const char* exploitation_signatures[] = {
    "buffer overflow", "injection", "shellcode", "ROP gadget", "privilege escalation"};

__attribute__((unused)) static const char* reconnaissance_signatures[] = {
    "nmap", "masscan", "shodan", "censys", "port scan", "service discovery"};
...
attacker_profile_t* create_attacker_profile(const char* ip) {
    ...
    if (ip)
        strncpy(profile->ip_address, ip, 31);
...
threat_assessment_t* assess_threat_level(attacker_profile_t** attackers, int attacker_count) {
    ...
    threat->should_trigger_rapid_morph =
        (threat->overall_threat_level > 0.6f) || is_coordinated_attack(attackers, attacker_count);
...
void update_attack_pattern(attack_pattern_t* pattern, const char* log_entry) {
    ...
    if (pattern->occurrence_count > 100)
        pattern->severity = 9;
    else if (pattern->occurrence_count > 50)
        pattern->severity = 8;
    else if (pattern->occurrence_count > 20)
        pattern->severity = 7;
...
void trigger_emergency_morph(void) {
    char msg[256];
    snprintf(
        msg, sizeof(msg), "EMERGENCY: Coordinated attack detected - triggering immediate morph!");
    log_event_level(LOG_WARN, msg);
    ...
    snprintf(signal_content,
             sizeof(signal_content),
...
void increase_morphing_frequency(uint32_t new_frequency_minutes) {
    if (new_frequency_minutes < 1)
        new_frequency_minutes = 1;
    if (new_frequency_minutes > 1440)
        new_frequency_minutes = 1440; // Max 24 hours

    current_morph_frequency_minutes = new_frequency_minutes;

    char msg[256];
    snprintf(
        msg, sizeof(msg), "Morphing frequency changed to every %u minutes", new_frequency_minutes);
    log_event_level(LOG_WARN, msg);
...
void simulate_errors_for_attacker(const char* ip) {
    ...
    snprintf(
        blocklist_entry, sizeof(blocklist_entry), "%s|%ld|coordinated_attack\n", ip, time(NULL));

    append_file(ATTACKER_BLOCKLIST, blocklist_entry);

    // Log what we're doing to this attacker
    snprintf(msg, sizeof(msg), "Attacker %s will now receive fake errors and slow responses", ip);
