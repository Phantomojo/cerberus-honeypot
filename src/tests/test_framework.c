...
void log_test_result(const char* test_name, test_result_t result, double time_ms) {
    ...
    switch (result) {
        case TEST_PASS: result_str = "PASS"; break;
        case TEST_FAIL: result_str = "FAIL"; break;
        case TEST_SKIP: result_str = "SKIP"; break;
...
test_result_t test_input_validation(test_context_t* ctx) {
    ...
    if (sec_validate_ip_address(good_ip) != SEC_VALID) {
        snprintf(ctx->failure_message, sizeof(ctx->failure_message),
                "Good IP validation failed: %s", good_ip);
        return TEST_FAIL;
    }
    
    if (sec_validate_ip_address(bad_ip) == SEC_VALID) {
        snprintf(ctx->failure_message, sizeof(ctx->failure_message),
                "Bad IP validation passed: %s", bad_ip);
        return TEST_FAIL;
    }
    ...
    if (sec_validate_filename(good_filename) != SEC_VALID) {
        snprintf(ctx->failure_message, sizeof(ctx->failure_message),
                "Good filename validation failed: %s", good_filename);
        return TEST_FAIL;
    }
    
    if (sec_validate_filename(bad_filename) == SEC_VALID) {
        snprintf(ctx->failure_message, sizeof(ctx->failure_message),
                "Bad filename validation passed: %s", bad_filename);
        return TEST_FAIL;
    }
