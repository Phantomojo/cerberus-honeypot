...
void test_safe_path_validation() {
    ...
    test_result("Accept build directory",
                is_safe_path("build/morph-state.txt"),
                "Valid build path");
    ...
    test_result("Reject /etc/passwd",
                !is_safe_path("/etc/passwd"),
                "System file access blocked");
    
    test_result("Reject /etc/shadow",
    ...
    test_result("Reject null byte",
                !is_safe_path("file.txt\x00.jpg"),
                "Null byte injection blocked");
    
    // Test 5: Reject empty/invalid paths
    test_result("Reject empty path",
                !is_safe_path(""),
                "Empty path rejected");
    
    test_result("Reject NULL path",
                !is_safe_path(NULL),
                "NULL path rejected");
}
...
void test_path_sanitization() {
    ...
    test_result("Remove ./ components",
                result == 0 && strstr(sanitized, "services") != NULL,
                sanitized);
...
void test_edge_cases() {
    ...
    test_result("Reject very long path",
                !is_safe_path(long_path),
                "Path length check working");
    
    // Test 2: Path with only dots
    test_result("Reject only dots",
                !is_safe_path("...."),
                "Invalid dot-only path rejected");
